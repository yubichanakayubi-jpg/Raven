...
from services.logs import enviar_log_bot, registrar_exclusao_temporaria, registrar_mencao_raven
...
async def limpar_mensagem_comando_gatilho(message, chave):
    ...
    try:
        ...
        await registrar_exclusao_temporaria(
            origem="Limpeza do comando interagir",
...
async def avisar_assumir_ticket_se_necessario(message, agora):
    ...
    mensagem_temp = await message.channel.send(
        f"{message.author.mention} se você vai atender esse ticket então {verbo} ele primeiro por favor!",
        allowed_mentions=discord.AllowedMentions(users=True, roles=False),
        delete_after=12,
    )
    await registrar_exclusao_temporaria(
        origem="Aviso temporario de assumir ticket",
        canal=message.channel,
        guild_ou_id=message.guild,
        autor=message.author,
        delay=12,
        mensagem_id=getattr(mensagem_temp, "id", None),
        conteudo=f"aviso para {verbo} ticket",
        alvo="aviso temporario",
    )
...
async def registrar_recrutamento_da_mensagem(message, agora, origem="chat"):
    ...
    async def enviar_resposta_recrutamento(titulo, descricao):
        ...
        await registrar_exclusao_temporaria(
            origem="Resposta temporaria de recrutamento",
            canal=message.channel,
            guild_ou_id=message.guild,
            autor=message.author,
            delay=10,
...
class EventsCog(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if bot_foi_mencionado and not eh_comando_prefixado:
            try:
                await registrar_mencao_raven(message, motivo="mencao direta")
            except Exception:
                pass
        ...
        if texto and not canal_atual_eh_ticket and mensagem_direcionada_raven:
            ...
            if motivo_direcao_raven == "reply" and mensagem_respondida is not None:
                ...
                if chave_resposta_unica and runtime.respostas_unicas_pergunta_diaria.get(chave_resposta_unica):
                    return
            ...
            if resposta:
                ...
                if mensagem_bot:
                    ...
                    if pergunta_diaria_message_id and chave_resposta_unica:
                        runtime.respostas_unicas_pergunta_diaria[chave_resposta_unica] = mensagem_bot.id
                        registrar_contexto_resposta_bot(
                            mensagem_bot,
